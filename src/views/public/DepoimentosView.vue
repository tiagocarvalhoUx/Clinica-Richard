<script setup lang="ts">
import Depoimentos from '@/components/sections/Depoimentos.vue';
import CtaAgendamento from '@/components/sections/CtaAgendamento.vue';
</script>

<template>
  <section class="overflow-hidden bg-gradient-hero">
    <div class="container-clinica grid gap-10 py-16 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:py-20">
      <div>
        <span class="eyebrow">Depoimentos</span>
        <h1 class="titulo-hero mt-4 text-balance">
          Confiança construída em cada explicação, retorno e sorriso.
        </h1>
        <p class="subtitulo mt-5 max-w-2xl">
          Reunimos relatos e sinais da experiência do paciente para mostrar o que
          realmente importa: clareza, acolhimento e segurança ao longo do cuidado.
        </p>
        <div class="mt-8 flex flex-wrap gap-3">
          <span class="rounded-full bg-white px-4 py-2 text-sm font-semibold text-primary-700 shadow-card">
            Atendimento humanizado
          </span>
          <span class="rounded-full bg-white px-4 py-2 text-sm font-semibold text-primary-700 shadow-card">
            Plano de cuidado claro
          </span>
          <span class="rounded-full bg-white px-4 py-2 text-sm font-semibold text-primary-700 shadow-card">
            Acompanhamento próximo
          </span>
        </div>
      </div>

      <div class="overflow-hidden rounded-[2rem] bg-white p-2 shadow-elevated ring-1 ring-slate-100">
        <img
          src="/image/depoimentos-pacientes.png"
          alt="Paciente em conversa acolhedora com profissional odontológico"
          class="aspect-[16/10] w-full rounded-[1.5rem] object-cover"
          loading="eager"
        />
      </div>
    </div>
  </section>

  <Depoimentos :limite="24" :mostrar-titulo="false" />
  <CtaAgendamento />
</template>
